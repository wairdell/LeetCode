...
public class BestTimeToBuyAndSellStockIV {
    ...
    public int maxProfit(int k, int[] prices) {
        return find(k, prices, 0, 0, 0, null);
    }

    public int find(int k, int[] prices, int total, int i, int n, Integer pre) {
        if (k == n) {
            return total;
        } else if (i == prices.length - 1) {
            return pre != null ? (total + prices[i] - pre) : total;
        }
        if (pre == null) {
            int branch1 = 0;
            //买
            if (i == 0 || prices[i] < prices[i - 1]) {
                branch1 = find(k, prices, total, i + 1, n, prices[i]);
            }
            //不买
            int branch2 = find(k, prices, total, i + 1, n, null);
            return Math.max(branch1, branch2);
        } else {
            int branch1 = 0;
            //卖
            if (prices[i] > pre && prices[i] > prices[i - 1]) {
                branch1 = find(k, prices, total + (prices[i] - pre), i + 1, n + 1, null);
            }
            //不卖
            int branch2 = find(k, prices, total, i + 1, n, pre);
            return Math.max(branch1, branch2);
        }
    }
