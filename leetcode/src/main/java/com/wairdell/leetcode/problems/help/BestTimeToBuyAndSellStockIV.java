...
public class BestTimeToBuyAndSellStockIV {
    ...
    public int maxProfit(int k, int[] prices) {
        ...
        //dp[i][0] 第一次持有，价值是负数的
        //dp[i][1] 第一次不持有，价值等于当前卖出价-上一次买进入价 dp[i][1] = max(prices[i] + dp[i - 1][0], dp[i - 1][1])
        //dp[i][2] 第二次持有，价值等于  第一次持有后卖出 - 当前价值 dp[i][2] = max(dp[i - 1][1] - prices[i] dp[i - 1][2])
        //dp[i][3] 第二次不持有，价值等于第二次持有后加上当前价值, dp[i][3] = max(dp[i - 1][2] + prices[i], dp[i - 1][3])
        //... 重复已上动作
